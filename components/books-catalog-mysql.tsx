...
import { Search, BookOpen, Plus, Trash2, Edit, Database } from "lucide-react"
import { getCurrentUser } from "@/lib/auth-mysql"
import { getAllBooks, addBook, updateBook, deleteBook, getBookCategories, searchBooks } from "@/lib/books-mysql"
...
export default function BooksCatalogMySQL() {
  ...
  const loadBooks = async () => {
    try {
      ...
      toast({
        ...
        description: `${booksData.length} kitap MySQL veritabanından yüklendi!`,
      ...
      toast({
        title: "Hata",
        description: "Kitaplar yüklenirken hata oluştu",
        variant: "destructive",
      })
    }
  }
  ...
  const handleAddBook = async () => {
    try {
      ...
      toast({
        title: "✅ Kitap Eklendi",
        description: `"${newBook.title}" başarıyla veritabanına eklendi!`,
        className: "bg-green-50 border-green-200 text-green-800",
      })
    } catch (error: any) {
  ...
  const handleEditBook = async () => {
    try {
      if (!editBook.title || !editBook.author || !editBook.isbn || !editingBook) {
        toast({ title: "Hata", description: "Lütfen tüm zorunlu alanları doldurun.", variant: "destructive" })
        return
      }
  ...
  const handleDeleteBook = async (bookId: number) => {
    try {
      ...
      toast({
        ...
        description: `"${bookToDelete?.title}" başarıyla veritabanından silindi!`,
